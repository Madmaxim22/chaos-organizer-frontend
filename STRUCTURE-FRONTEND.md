...
3. **Настройка линтинга и форматирования**
   - Установить ESLint с конфигурацией custom ([FRONTEND.md, раздел «Линтинг»](#)).
...
9. **Стилевая архитектура**
   - Создать базовые глобальные стили, переменные (цвета, шрифты) в `./src/css/style.css`.
...
10. **Компонент сообщения (`Message`)**
    - Реализовать отображение текста, ссылок ([FRONTEND.md, раздел «Отправка и отображение сообщений»](#)).
    - Добавить кнопки действий (скачать, закрепить, добавить в избранное, удалить).

11. **Панель ввода (`InputPanel`)**
    - Текстовое поле с поддержкой переноса строк.
    - Кнопки прикрепления файлов, запись голосовых сообщений и видео (создай отдельные классы для записи аудио и видео).
    - Отображение прикрепленных файлов, возможность их удаления (создай отдельный класс для работы с файлами).
    - При использовании Drag & Drop показывать модальное окно которое будет использоваться как область для загрузки файлов ([FRONTEND.md, раздел «Загрузка файлов»](#)).

12. **Боковое меню (`Sidebar`)**
    - Навигация по категориям вложений (аудио, видео, изображения, избранное, другие файлы).
    - Ссылки на настройки, экспорт/импорт.
    - Скрывается и показывается по нажатию.
