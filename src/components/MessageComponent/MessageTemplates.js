/**
 * Шаблоны и утилиты для рендеринга HTML-разметки сообщений.
 * formatters: { escapeHTML, formatBytes, yandexMapsApiKey?: string }
 */
import LocationService from '@/services/LocationService.js';
import { getUnicode as getEmojiUnicode } from '@/services/EmojiShortcodeResolver.js';
import {
...
/** Рендерит отформатированный текст сообщения (markdown-like, жирный, курсив, эмодзи). */
export function renderFormattedText(text, messageId, formatters) {
  ...
  let formatted = withLinks
    ...
    .replace(/:([a-z0-9_+-]+):/g, (match, name) => getEmojiUnicode(name) || match);
  ...
  const toggleButton = shouldTruncate
    ? `<button type="button" class="text-toggle-btn" data-message-id="${messageId}"
      aria-label="Показать полный текст сообщения">Показать больше</button>`
    : '';
...
/**
 * Рендерит карточку вложения (превью, имя, размер).
 ...
 * @param {{ isEncrypted?: boolean }} [options] - если true, показывается зашифрованное превью
 * @returns {string}
 */
export function renderAttachment(metadata, baseUrl, formatters, options = {}) {
  ...
  const rawFilename = metadata.fileName || 'Без имени';
...
/**
 * Рендерит превью карты по координатам из содержимого сообщения.
 * Изображение запрашивается через эндпоинт /api/static-map (карты Yandex или другой провайдер).
  ...
  return `
      <div class="message-map-preview">
        <a href="${mapLink}" target="_blank" rel="noopener noreferrer"
          class="message-map-preview-link" title="Открыть карту в Яндекс.Картах">
          <img src="${imgUrl}" alt="Карта местоположения: ${latitude.toFixed(4)}, ${longitude.toFixed(4)}"
            class="message-map-preview-img" loading="lazy">
        </a>
        <span class="message-map-preview-caption">Местоположение</span>
      </div>`;
}

/**
 * Собирает полный HTML одного сообщения.
  ...
  if (message.encrypted && decryptedContent == null) {
    bodyHTML = `
      <div class="message-body-encrypted" data-encrypted-placeholder="true">
        <p class="encrypted-placeholder-text">Содержимое сообщения зашифровано</p>
        <button type="button" class="action-button decrypt-btn" data-action="decrypt"
          aria-label="Расшифровать и показать текст сообщения">
          <img src="${LockOffUrl}" alt="" aria-hidden="true" class="decrypt-btn-icon"> Расшифровать
        </button>
      </div>`;
